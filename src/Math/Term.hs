...
data Term = U | Lambda String Term Term | Var String Term
    ...
    | Prim PrimConst | Def Term [Term] | Match [(Term, Term)]
...
instance Show Term where
    ...
    show (Match terms) = "Match : \n" ++ concatMap go terms where
        go (pattern, t) = show pattern ++ " => " ++ show t
...
relation a b = if a == b then EQUIV else goRelation (alphaReduce a) (alphaReduce b) where
    ...
    goRelation (Match termMap) (Pi (Var x a) b) = if a == (go termMap) && b == go2 termMap then SUBTYPE else NOTEQ where
        go [(x, y)] = x
        go ((x, y) : zs) = Coprod x (go zs) 
        go2 [(x, y)] = y
        go2 ((x, y) : zs) = Coprod y (go2 zs) 
    goRelation (Pi (Var x a) b) (Match termMap) = if goRelation (Match termMap) (Pi (Var x a) b) == SUBTYPE then SUPERTYPE else NOTEQ 
